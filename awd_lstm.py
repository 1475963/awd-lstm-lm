...
# TODO first implementation: raw code from merity
# TODO second pass: adapt this to how our LSTM works

def embedded_dropout(embed, words, dropout=.1, scale=None):
	if dropout:
...
class RNNModel(nn.Module):
	def __init__(
			self, ntoken, ninp, nhid, nlayers,
			dropout=.5, dropouth=.5, dropouti=.5, dropoute=.1, wdrop=0, tie_weights=False,
	):
		...
		self.rnns = [
			nn.LSTM(ninp if l == 0 else nhid, nhid if l != nlayers - 1 else (ninp if tie_weights else nhid), 1, dropout=0)
